...
function HeaderCommandButton({ children, primary = false }) { return <button className={`button-shine header-command ${primary ? "primary" : ""}`}><span className="button-sweep" /><span className="button-content">{children}</span></button>; }
function ShieldButton({ children, light = false }) { return <button className={`button-shine shield-button ${light ? "light" : ""}`}><span className="button-sweep" /><span className="button-content">{children}</span></button>; }
...
export default function App() {
  return <div className="site-shell"><PremiumEffects /><div className="site-bg" /><div className="site-grid" />
    ...
    <section className="section"><div className="container"><DecreeBanner eyebrow="FINAL IDENTITY" title="A persistent player-driven war MMO." text="Players level, fight, lose gear, control territory, govern cities, compete over dynamic resources, and reshape a constantly shifting ancient world." cta={<div className="final-ctas"><ShieldButton light>Play Alpha</ShieldButton><ShieldButton>View Whitepaper</ShieldButton></div>}/></div></section></main>
